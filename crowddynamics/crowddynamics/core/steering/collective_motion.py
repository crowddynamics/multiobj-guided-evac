...
def leader_follower_interaction_brute(
        ...
        target, index_leader, obstacles, sight, is_active):
    ...
    leaders = indices[is_leader & is_active]
...
def leader_follower_interaction(
    ...
    # Check if there is a guide to follow.
    # We shouldn't do this for following agents that already have a guide.
    target, has_strategy = leader_follower_interaction_brute(
        agents['is_follower'] & (agents['index_leader']==-1), agents['is_leader'], agents['position'],
        agents['velocity'], weight_position_leader, phi,
        agents['target'], agents['index_leader'], obstacles, sight, agents['active'])
    # Hasn't seen a guide and is a follower
    is_lost = ~has_strategy & agents['is_follower'] & (agents['index_leader'] == -1)
    # Has just seen a guide and is a follower
